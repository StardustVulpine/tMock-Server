...
namespace tmockserver::packets {
    FatalError::FatalError (const NetworkTextMode textMode, const std::string_view text) : BasePacket(text.size()+5, PacketType::FATAL_ERROR)
    {
    ...
    void FatalError::Print() const {
        PrintPacketHead(PacketDirection::SEND);
