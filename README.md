...
|   5+   |     6+      |      1       |                string                 | Text Message Content | Error Message's Text Content. Can be any message when used with `LITERAL` Text Mode. Terraria uses `LOCALIZATION_KEY` for most messages since this support errors display in multiple languages that are accessed by KEYS specified inside [json file](res/Terraria.Localization.Content.en-US.json). |
...
![Diagram showing communication schema](/res/TNP_Communication_Map.drawio.svg)
